...
%   LDA(linear discriminant analysis),即Fisher 判别分析
%   class = Class_fisher(Sampledata,Training,group) 根据训练样本Training构造判别式，
...
%   同的行数. Class_fisher函数把group中的NaN或空字符串作为缺失数据，从而忽略Training
%   中相应的观测. class中的每个元素指定了Sampledata中的相应观测所判归的类,它和
%   group具有相同的数据类型. 
%
%   class = Class_fisher(Sampledata,Training,group,contri) 根据累积贡献率不低于
%   contri，确定需要使用的判别式个数，默认情况下，使用所有判别式进行判别. contri
%   是一个在(0, 1]区间内取值的标量，用来指定累积贡献率的下限. 
%
%   [class, TabCoef] = Class_fisher(...)以表格形式返回所用判别式的系数向量，若contri
%   取值为1，则返回所有判别式的系数向量. TabCoef是一个元胞数组，形如
%       'Variable'    'can1'       'can2'   
%       'x1'          [-0.2087]    [ 0.0065]
%       'x2'          [-0.3862]    [ 0.5866]
%       'x3'          [ 0.5540]    [-0.2526]
...
%   [class, TabCoef, TabL] = Class_fisher(...)以表格形式返回所有特征值，贡献率，累积
%   贡献率等. TabL是一个元胞数组，形如
%       'Eigenvalue'    'Difference'    'Proportion'    'Cumulative'
%       [   32.1919]    [   31.9065]    [    0.9912]    [    0.9912]
%       [    0.2854]              []    [    0.0088]    [         1]
%
%   [class, TabCoef, TabL, TabConf] = Class_fisher(...)以表格形式返回混淆矩阵（包含总
%   的分类信息的矩阵）. TabConf是一个元胞数组，形如
%       'From/To'       'setosa'    'versicolor'    'virginica'
%       'setosa'        [    50]    [         0]    [        0]
%       'versicolor'    [     0]    [        48]    [        2]
%       'virginica'     [     0]    [         1]    [       49]
%
%   [class, TabCoef, TabL, TabConf, TabErrMat] = Class_fisher(...)以表格形式返回误判矩阵. 
%   TabErrMat是一个元胞数组，形如
%       'Obj'    'From'          'To'        
...
%
%   [class, TabCoef, TabL, TabConf, TabErrMat, TabG] = Class_fisher(...)将所用判别式作用
...
%   [class, TabCan, TabL, TabCon, TabM, TabG, TrainScore] = Class_fisher(...)返回
